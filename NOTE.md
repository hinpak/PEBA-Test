# Background
PEBA as a better alternative to TPU for 3D Printing

# Objective
print: PneuNet actuator, a compilant mechanism, an object of your choice

# Filaments & Print Settings
Drying Setting: 80°C / 8h

## 1. [eSUN PEBA 90A](https://detail.tmall.com/item.htm?id=982039037506&mi_id=0000wG3tWqlvB4PABsVTj7SUnv2IRtrrQPy8nm-xbsea0Fs&spm=tbpc.boughtlist.suborder_itemtitle.1.6a7c2e8daRjHZ8&sku_properties=205232072%3A93500215)

| Parameter | Recommended Value | 
| -------- | -------- |
| Nozzle Temperature | 230-260°C |
| Print Bed Temperature | 60-90°C |
| Print speed | 25-100mm/s |
| Infill Density | 10-40% |
| Fan | 10-50% |

<!-- [Actual Printing Setting] -->

## 2. [Siraya Tech PEBA 95A](https://item.taobao.com/item.htm?id=942425589849&mi_id=000003CTrExh_DXCL_G-TLEJRc4K7rkSr_-eD3a74L18XOU&spm=tbpc.boughtlist.suborder_itemtitle.1.6a7c2e8daRjHZ8&skuId=6007898674796)

| Parameter | Recommended Value | 
| -------- | -------- |
| Nozzle Temperature | 230-250°C |
| Print Bed Temperature | 90-100°C |
| Print speed | 40-60mm/s |
| Fan | No / Low Speed |
| Layer height | 0.2mm |

Merchant Reference: /TestModel/3mf/Reference/Siraya Tech PEBA P1S 0.4mm 20250724.3mf

<!-- [Actual Printing Setting] -->

## 3. [Xibochuan PEBA 90A](https://detail.tmall.com/item.htm?id=905487893484&mi_id=0000Nr_DXbvr9vDtqZzqFl1Fbb82_VmxLbw9_LYcAZtG_ik&spm=tbpc.boughtlist.suborder_itemtitle.1.6a7c2e8daRjHZ8)

| Parameter | Recommended Value | 
| -------- | -------- |
| Nozzle Temperature | 230-250°C |
| Print Bed Temperature | 60°C (without ) ; 100-110°C (with)|
| Print speed | 5mm^3/s |
| Retraction | 0.4-1.6mm |

Merchant recommends directly use generic tpu profile.

<!-- [Actual Printing Setting] -->